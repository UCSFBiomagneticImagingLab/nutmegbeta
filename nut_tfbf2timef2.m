function nut_tfbf2timef2(sessionfile,algo,controlperiod)
...
if(size(timewins,1)>1)
    ...
    if(timewins(1,2) == timewins(2,1)) % no overlap
        ...
        tw = [timepts-(timestep/2) timepts+(timestep/2)];
        beam.timewindow=tw; %we think this should work for everyone, however may or may be equal to tw1 depending what you've done
    end
else
    beam.timewindow = timewins;
end
...
%         end
    ...
    for timebin=1:size(timepts,1)
        beam.s{1}(:,timebin,freqbin) = Sact(:,timebin);
        beam.s{2}(:,timebin,freqbin) = Scon(:,timebin);
        if(noiseflag)
            beam.s{3}(:,timebin,freqbin) = noise(:,timebin);
        end
    end
%     for timebin=1:size(timepts,1)
