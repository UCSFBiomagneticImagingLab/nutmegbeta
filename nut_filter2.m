...
% filttype   'notch', 'bp' (bandpass), 'low', 'high', 'lowhigh' (two-pass)
%            or 'baseline_only'
...
switch(lower(filttype))
    case 'notch'
        ...
        data=runfilt(b,a,data);
    case 'baseline_only'
        return
    case 'lowhigh'
        [b,a] = filtdef(filtclass,filtorder,[],hifreq,fs,'low',size(data,1));
        data=runfilt(b,a,data);  %lowpass filter
        [b,a] = filtdef(filtclass,filtorder,lofreq,[],fs,'high',size(data,1));
        data=runfilt(b,a,data);  %highpass filter        
    otherwise
...
%-----------------------------
function data=runfilt(b,a,data)
% data should be double before going into filter;
