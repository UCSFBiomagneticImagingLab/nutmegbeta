function CC = fcm_correctspatialleakage(CC,filtfile,Wfile)
% Correct for spatial leakage by performing a Monte Carlo simulation of
% colored noise. At each iteration of this simulation, white noise of
% equal length and channel numbers as the real artifact-free sensor data 
% is generated, colored by bandpass filtering to the same frequency band as
% in real data, and projected to source space with the same adaptive spatial 
% filter computed from real data. FC is then obtained from filtered and 
% projected noise. This yields, a distribution of spurious interactions 
% produced by noise, which can be compared to the values obtained by real 
% data by computing z-scores.
%
% CC = fcm_correctspatialleakage(CC,W)

numperm = 30;

s = regexp(filtfile, '[_]+', 'split');
frq = sscanf(s{end},'%fto%fHz')';
fi = s{end-1};
