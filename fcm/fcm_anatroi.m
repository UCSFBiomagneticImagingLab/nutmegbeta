...
case 'jul'
    ...
    list = {MAP.name};
    if strcmp('-list',label{1})
        roi = sortrows(char(list));
        return
    end
    
    roi.MNIvoxels=[];
    for k=1:length(label)
        selLR = any(strcmpi(label{k}(end-1:end),{' L' ' R'}));
        if selLR
            lab=label{k}(1:end-2);
        else
            lab=label{k};
        end
        v = find(strcmpi(lab,list));
        if isempty(v), error('label %s not found',label{k}), end 
        if selLR
            if strcmpi(label{k}(end),'L')
                coord = MAP(v).XYZmm(:,MAP(v).LR<0)';
            else 
                coord = MAP(v).XYZmm(:,MAP(v).LR>0)';
            end
        else
        ...
        roi.MNIvoxels = [roi.MNIvoxels;coord];
    end
    roi.MNIvoxels = unique(roi.MNIvoxels,'rows');
    voxelsize=[1 1 1];
...
if ~exist('roi','var') || ~isfield(roi,'MNIvoxels')
